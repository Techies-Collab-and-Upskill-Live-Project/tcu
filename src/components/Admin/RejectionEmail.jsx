...
const RejectionEmail = ({ className, formClass, inputClass }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${baseUrl}/internship/send-rejection-mail/`,
        {
          recipients,
        }
      );
    ...
    >
      <div className="flex flex-col justify-center items-center bg-[#181818] max-w-[720px] w-full p-5 rounded-[12px]">
        <p className="md:text-[36px] text-[16px] font-[700] text-[#ffffff]">
          TCU 3.0 Rejection Email Form
        </p>
        <p className="md:text-[20px] text-[10px] font-[400] text-[#9c9c9c]">
          Send emails to unsuccessful applicants. Required fields are indicated
      ...
      >
        ...
        >
          {isLoading
            ? "Sending..."
            : `Send Rejection ${recipients.length > 1 ? "Emails" : "Email"}`}
        </Button>
      </form>
    </div>
  );
